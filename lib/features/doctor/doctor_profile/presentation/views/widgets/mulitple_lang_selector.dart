...
class MuiltipleValueLangsSelector extends StatefulWidget {
  ...
  @override
  State<MuiltipleValueLangsSelector> createState() =>
      _MuiltipleValueLangsSelectorState();
}

class _MuiltipleValueLangsSelectorState
    extends State<MuiltipleValueLangsSelector> {
  ...
  void initState() {
    ...
    selectedLangs =
        (doctorModel?.langs ?? []).map((lang) => lang.name ?? '').toList();
          ...
          child: BlocBuilder<LanguageCubit, LanguageState>(
            builder: (context, state) {
              return CustomTextFormFieldWithSuggestions(
                label: 'Languages',
                suggestions: state.languageModels
                        ?.map((lang) => lang.name ?? '')
                        .toList() ??
                    [],
